...
if ($pangkat == $cekpangkat && $jabatan == null) {
    $sql = mysqli_query($con, "SELECT * FROM gadik a JOIN pangkat b ON a.id_pangkat = b.id_pangkat JOIN jabatan c ON a.id_jabatan = c.id_jabatan WHERE a.id_pangkat = '$pangkat' ORDER BY tmt DESC");

    $dt = $con->query("SELECT * FROM pangkat WHERE id_pangkat = '$pangkat'")->fetch_array();
    $label = 'LAPORAN DATA GADIK <br> Pangkat : ' . $dt['nm_pangkat'];
} else if ($jabatan == $cekjabatan && $pangkat == null) {
    $sql = mysqli_query($con, "SELECT * FROM gadik a JOIN pangkat b ON a.id_pangkat = b.id_pangkat JOIN jabatan c ON a.id_jabatan = c.id_jabatan WHERE a.id_jabatan = '$jabatan' ORDER BY tmt DESC");

    $dt = $con->query("SELECT * FROM jabatan WHERE id_jabatan = '$jabatan'")->fetch_array();
    $label = 'LAPORAN DATA GADIK <br> Jabatan : ' . $dt['nm_jabatan'];
} else if ($pangkat == $cekpangkat && $jabatan == $cekjabatan) {
    $sql = mysqli_query($con, "SELECT * FROM gadik a JOIN pangkat b ON a.id_pangkat = b.id_pangkat JOIN jabatan c ON a.id_jabatan = c.id_jabatan WHERE a.id_pangkat = '$pangkat' AND a.id_jabatan = '$jabatan' ORDER BY tmt DESC");
    ...
    $label = 'LAPORAN DATA GADIK <br> Pangkat : ' . $dt1['nm_pangkat'] . '<br> Jabatan : ' . $dt2['nm_jabatan'];
} else {
    $sql = mysqli_query($con, "SELECT * FROM gadik a JOIN pangkat b ON a.id_pangkat = b.id_pangkat JOIN jabatan c ON a.id_jabatan = c.id_jabatan ORDER BY tmt DESC");
    $label = 'LAPORAN DATA GADIK';
}
